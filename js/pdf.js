...
switch (fam) {
case ‘header’:
return `<div style="position:absolute;inset:0;background:${C.bg}"></div> <div style="position:absolute;top:0;left:0;right:0;height:${h};background:linear-gradient(135deg,${C.a1},${C.a2})"></div>`;
case ‘glow’:
return `<div style="position:absolute;inset:0;background:${C.bg}"></div> <div style="position:absolute;top:-120px;right:-120px;width:550px;height:550px;border-radius:50%;background:radial-gradient(circle,${C.a1}50 0%,transparent 65%)"></div> <div style="position:absolute;bottom:-90px;left:-90px;width:420px;height:420px;border-radius:50%;background:radial-gradient(circle,${C.a2}40 0%,transparent 65%)"></div>`;
case ‘line’:
return `<div style="position:absolute;inset:0;background:${C.bg}"></div> <div style="position:absolute;left:80px;right:80px;top:49%;height:1px;background:linear-gradient(90deg,transparent,${C.a1},${C.a2},transparent)"></div>`;
case ‘bauhaus’:
return `<div style="position:absolute;inset:0;background:${C.bg}"></div> <div style="position:absolute;right:-120px;top:50%;transform:translateY(-50%);width:560px;height:560px;border-radius:50%;background:${C.a1};opacity:.8"></div> <div style="position:absolute;right:-60px;top:50%;transform:translateY(-50%);width:420px;height:420px;border-radius:50%;background:${C.bg};opacity:.28"></div>`;
case ‘noir’:
return `<div style="position:absolute;inset:0;background:#050505"></div> <div style="position:absolute;left:0;right:0;top:${isCover?'55%':'43%'};height:10px;background:linear-gradient(90deg,${C.a1},${C.a2})"></div> <div style="position:absolute;left:0;right:0;top:${isCover?'52%':'40%'};height:20px;background:radial-gradient(ellipse at 50% 100%,${C.a1}55,transparent 80%)"></div>`;
default: // classic
return `<div style="position:absolute;inset:0;background:${C.bg}"></div> <div style="position:absolute;left:0;top:0;bottom:0;width:6px;background:linear-gradient(180deg,${C.a2},${C.a1})"></div> <div style="position:absolute;left:0;bottom:0;right:0;height:4px;background:linear-gradient(90deg,${C.a1},${C.a2},transparent)"></div>`;
...
let decor;
if (fam === ‘classic’) {
decor = `<div style="position:absolute;inset:0;background:${C.bg}"></div> <div style="position:absolute;right:0;top:0;width:40%;bottom:0;background:${C.bg2}"></div> <div style="position:absolute;left:0;bottom:0;width:60%;height:5px;background:linear-gradient(90deg,${C.a1},${C.a2})"></div>`;
} else if (fam === ‘noir’) {
decor = `<div style="position:absolute;inset:0;background:#050505"></div> <div style="position:absolute;left:0;right:0;top:60%;height:10px;background:linear-gradient(90deg,${C.a1},${C.a2})"></div>`;
} else if (fam === ‘aqua’) {
decor = `${_decor(C, fam, true)} <!-- painel direito com véu de vidro-água --> <div style="position:absolute;right:0;top:0;width:38%;bottom:0;background:${C.bg2};opacity:.55"></div> <div style="position:absolute;right:38%;top:0;width:4px;bottom:0;background:linear-gradient(180deg,${C.a1},${C.tl},${C.a2})"></div>`;
} else if (fam === ‘mesh’) {
decor = `${_decor(C, fam, true)} <!-- card de leitura semi-transparente --> <div style="position:absolute;left:0;top:0;width:60%;bottom:0;background:${C.bg2};opacity:.36"></div> <div style="position:absolute;left:0;top:0;width:4px;bottom:0;background:linear-gradient(180deg,${C.a1},${C.a2})"></div>`;
} else if (fam === ‘void’) {
...
let accentRule;
if (fam === ‘aqua’) {
accentRule = `<div style="display:flex;gap:0;margin-bottom:22px"> <div style="width:56px;height:4px;background:${C.a1};border-radius:2px 0 0 2px"></div> <div style="width:32px;height:4px;background:${C.tl}"></div> <div style="width:18px;height:4px;background:${C.a2};border-radius:0 2px 2px 0"></div> </div>`;
} else if (fam === ‘mesh’) {
accentRule = `<div style="display:flex;align-items:center;gap:10px;margin-bottom:22px"> <div style="width:12px;height:12px;border-radius:50%;background:${C.a1}"></div> <div style="flex:1;max-width:120px;height:3px;background:linear-gradient(90deg,${C.a2},transparent);border-radius:2px"></div> </div>`;
} else if (fam === ‘void’) {
...
const body = ` <div style="margin-top:auto;margin-bottom:auto"> <div style="font-size:12px;letter-spacing:4px;text-transform:uppercase;color:${mc};margin-bottom:22px">${safe(id.instName,'Organização')}</div> <div style="font-size:${titlePx}px;font-weight:${fam==='void'?300:800};line-height:1.05;letter-spacing:${fam==='void'?'-1px':'-2px'};color:${tc};margin-bottom:22px;max-width:780px">${safe(id.presTitle,'Título da Apresentação')}</div> ${accentRule} <div style="font-size:20px;color:${mc};max-width:620px;line-height:1.5">${safe(id.presSubtitle || id.presDate,'')}</div> </div> <div style="margin-top:auto;display:flex;align-items:center;gap:14px"> <div style="width:34px;height:34px;border-radius:8px;background:${C.a1};display:flex;align-items:center;justify-content:center;font-size:18px;font-weight:900;color:${C.bg}">W</div> <span style="font-size:14px;color:${mc}">${safe(id.presDate,'')}</span> </div>`;
...
return `<div style="display:flex;align-items:center;gap:20px;padding:13px 0;border-bottom:1px solid ${C.a1}18"> <span style="font-size:26px;font-weight:800;color:${C.a1};min-width:46px;font-family:monospace">${n}</span> <div> <div style="font-size:19px;font-weight:600;color:${C.txt};margin-bottom:2px">${safe(p.name,`Projeto ${n}`)}</div> <div style="font-size:13px;color:${C.mu}">${safe((p.objetivo||'').slice(0,90),'')}</div> </div> </div>`;
}).join(’’);
...
const full = `<div style="position:absolute;inset:0;background:linear-gradient(135deg,${C.a1},${C.a2})"></div> <div style="position:absolute;inset:0;background:${C.bg}bb"></div>`;

// Void: divisor com preto absoluto + 1 acento + peso leve
if (fam === ‘void’) {
const body = ` <div style="margin-top:auto;margin-bottom:auto"> <div style="font-size:12px;letter-spacing:5px;text-transform:uppercase;color:${C.a1};margin-bottom:16px">${G.mode==='single'?'PROJETO':`PROJETO ${n}`}</div> <div style="font-size:64px;font-weight:300;line-height:1.05;letter-spacing:-1px;color:#fff;margin-bottom:18px">${safe(proj.name,`Projeto ${n}`)}</div> <div style="width:180px;height:1.5px;background:${C.a1};margin-bottom:18px"></div> <div style="font-size:18px;color:${C.a1};opacity:.75;max-width:700px;line-height:1.5">${safe((proj.objetivo||'').slice(0,130),'')}</div> </div>`;
return _shell(_decor(C, fam, false), body, C, fam, ‘70px’);
}

const body = ` <div style="margin-top:auto;margin-bottom:auto"> <div style="font-size:110px;font-weight:900;color:${C.a1};opacity:.12;line-height:1;margin-bottom:-34px;font-family:monospace">${n}</div> <div style="font-size:60px;font-weight:800;color:${C.txt};letter-spacing:-2px;line-height:1.1;margin-bottom:18px">${safe(proj.name,`Projeto ${n}`)}</div> <div style="width:56px;height:3px;background:${C.a2};border-radius:2px;margin-bottom:18px"></div> <div style="font-size:20px;color:${C.mu};max-width:700px;line-height:1.5">${safe((proj.objetivo||'').slice(0,130),'')}</div> </div>`;
return _shell(full, body, C, ‘divisor’, ‘70px’);
}

function _sObjetivo(C, fam, proj) {
const body = ` ${_eye('Objetivo',C,fam)} ${_h2(safe(proj.name,'Projeto'),C)}${_rule(C)} <div style="flex:1;${_cardStyle(C)}overflow:hidden"> <p style="font-size:21px;color:${C.txt};line-height:1.7;opacity:.85">${safe(proj.objetivo,'Objetivo não definido.')}</p> </div>`;
return _shell(_decor(C,fam), body, C, fam);
}
...
`<div style="background:${C.bg2};border:1px solid ${C.a1}1a;border-radius:10px;padding:20px;text-align:center"> <div style="width:46px;height:46px;border-radius:50%;background:linear-gradient(135deg,${C.a1},${C.a2});margin:0 auto 10px;display:flex;align-items:center;justify-content:center;font-size:20px;font-weight:800;color:${C.bg}">${(m.nome||'?')[0].toUpperCase()}</div> <div style="font-size:15px;font-weight:600;color:${C.txt};margin-bottom:4px">${safe(m.nome,'—')}</div> <div style="font-size:12px;color:${C.a1};margin-bottom:6px">${safe(m.cargo,'')}</div> <div style="font-size:12px;color:${C.mu};line-height:1.4">${safe(m.resp,'')}</div> </div>`).join(’’) || `<p style="color:${C.mu}">Equipe não cadastrada.</p>`;
const body = `${_eye('Equipe',C,fam)}${_h2(safe(proj.name,'Projeto'),C)}${_rule(C)} <div style="display:grid;grid-template-columns:repeat(${n},1fr);gap:14px;flex:1">${cards}</div>`;
return _shell(_decor(C,fam), body, C, fam);
}
...
`<div style="display:flex;align-items:center;gap:14px;padding:9px 0;border-bottom:1px solid ${C.a1}14"> <span style="font-size:11px;color:${C.a1};min-width:22px;font-family:monospace;font-weight:700">${String(i+1).padStart(2,'0')}</span> <span style="flex:1;font-size:15px;color:${C.txt}">${safe(e.nome,'—')}</span> <span style="font-size:12px;color:${C.mu};min-width:110px">${safe(e.inicio,'')}${e.fim?' → '+e.fim:''}</span> ${_badge(safe(e.status,'planejada'), sc[(e.status||'').toLowerCase()]||'neutral')} </div>`).join(’’) || `<p style="color:${C.mu}">Etapas não cadastradas.</p>`;
...
`<div style="display:flex;align-items:center;gap:18px;padding:10px 0;border-bottom:1px solid ${C.a1}14"> <div style="width:9px;height:9px;border-radius:50%;background:${C.a1};flex-shrink:0"></div> <span style="flex:1;font-size:16px;color:${C.txt}">${safe(m.nome,'—')}</span> <span style="font-size:12px;color:${C.mu};min-width:100px;text-align:right">${safe(m.data,'')}</span> ${_badge(safe(m.status,'planejado'), sc[(m.status||'').toLowerCase()]||'neutral')} </div>`).join(’’) || `<p style="color:${C.mu}">Marcos não cadastrados.</p>`;
...
return `<div style="background:${C.bg2};border:1px solid ${C.a1}1a;border-radius:10px;padding:22px 26px"> <div style="font-size:11px;color:${C.mu};font-weight:700;letter-spacing:1.5px;text-transform:uppercase;margin-bottom:8px">${safe(k.nome,'KPI')}</div> <div style="display:flex;align-items:baseline;gap:6px;margin-bottom:10px"> <span style="font-size:46px;font-weight:900;color:${C.a1};line-height:1">${safe(k.real,'—')}</span> <span style="font-size:15px;color:${C.mu}">${safe(k.unidade,'')}</span> </div> <div style="font-size:13px;color:${C.mu};margin-bottom:10px">Meta: <b style="color:${C.txt}">${safe(k.meta,'—')} ${safe(k.unidade,'')}</b></div> ${pct!==null?`<div style="height:4px;background:${C.bg};border-radius:2px"><div style="height:4px;width:${pct}%;background:linear-gradient(90deg,${C.a1},${C.a2});border-radius:2px"></div></div>`:''} </div>`;
}).join(’’) || `<p style="color:${C.mu}">Indicadores não cadastrados.</p>`;
const body = `${_eye('Indicadores KPI',C,fam)}${_h2(safe(proj.name,'Projeto'),C)}${_rule(C)} <div style="display:grid;grid-template-columns:repeat(${n},1fr);gap:14px;flex:1">${cards}</div>`;
return _shell(_decor(C,fam), body, C, fam);
}
...
return `<div style="background:${C.bg2};border:1px solid ${C.a1}1a;border-radius:10px;padding:26px;display:flex;flex-direction:column;gap:8px"> <div style="font-size:11px;color:${C.mu};font-weight:700;letter-spacing:1.5px;text-transform:uppercase">${safe(r.titulo,'Resultado')}</div> <div style="font-size:52px;font-weight:900;color:${C.a1};line-height:1">${safe(r.valor,'—')}</div> <div style="font-size:15px;color:${C.mu}">${safe(r.unidade,'')}</div> ${r.variacao!=null?`<div style="font-size:15px;color:${vc};font-weight:700">${vs} ${Math.abs(r.variacao)}%</div>`:''} </div>`;
}).join(’’) || `<p style="color:${C.mu}">Resultados não cadastrados.</p>`;
const body = `${_eye('Resultados',C,fam)}${_h2(safe(proj.name,'Projeto'),C)}${_rule(C)} <div style="display:grid;grid-template-columns:repeat(${n},1fr);gap:14px;flex:1">${cards}</div>`;
return _shell(_decor(C,fam), body, C, fam);
}

function _sAnteDepois(C, fam, proj) {
const ad = proj.antesdepois || {};
const body = `${_eye('Antes & Depois',C,fam)}${_h2(safe(proj.name,'Projeto'),C)}${_rule(C)} <div style="display:grid;grid-template-columns:1fr 1fr;gap:18px;flex:1"> <div style="background:${C.bg2};border:1px solid ${C.a1}18;border-radius:10px;padding:28px"> <div style="font-size:11px;color:${C.mu};font-weight:700;letter-spacing:2.5px;margin-bottom:16px">ANTES</div> <p style="font-size:17px;color:${C.txt};line-height:1.65;opacity:.82">${safe(ad.antes_txt,'Situação anterior não descrita.')}</p> </div> <div style="background:${C.bg2};border:1px solid ${C.a2}44;border-top:3px solid ${C.a2};border-radius:10px;padding:28px"> <div style="font-size:11px;color:${C.a2};font-weight:700;letter-spacing:2.5px;margin-bottom:16px">DEPOIS</div> <p style="font-size:17px;color:${C.txt};line-height:1.65;opacity:.82">${safe(ad.depois_txt,'Situação posterior não descrita.')}</p> </div> </div>`;
return _shell(_decor(C,fam), body, C, fam);
}
...
`<div style="display:flex;align-items:flex-start;gap:14px;padding:11px 0;border-bottom:1px solid ${C.a1}14"> <div style="flex:1"> <div style="font-size:15px;color:${C.txt};margin-bottom:3px">${safe(r.descricao,'—')}</div> <div style="font-size:12px;color:${C.mu}">${safe(r.mitigacao,'')}</div> </div> <div style="display:flex;gap:7px;flex-shrink:0"> ${_badge('P:'+safe(r.probabilidade,'?'), pc(r.probabilidade))} ${_badge('I:'+safe(r.impacto,'?'), pc(r.impacto))} </div> </div>`).join(’’) || `<p style="color:${C.mu}">Riscos não cadastrados.</p>`;
...
`<div style="display:flex;gap:16px;padding:13px 0;border-bottom:1px solid ${C.a1}14"> <span style="font-size:24px;font-weight:900;color:${C.a1};opacity:.35;min-width:34px;font-family:monospace">${i+1}</span> <div> <div style="font-size:16px;font-weight:600;color:${C.txt};margin-bottom:4px">${safe(l.titulo,'—')}</div> <div style="font-size:13px;color:${C.mu};line-height:1.5">${safe(l.descricao,'')}</div> </div> </div>`).join(’’) || `<p style="color:${C.mu}">Lições não cadastradas.</p>`;
...
`<div style="background:${C.bg2};border-left:3px solid ${C.a1};border-radius:0 8px 8px 0;padding:14px 20px;margin-bottom:11px"> <div style="font-size:16px;font-weight:600;color:${C.txt};margin-bottom:4px">${safe(d.titulo,'—')}</div> <div style="font-size:13px;color:${C.mu};line-height:1.5">${safe(d.descricao,'')}</div> </div>`).join(’’) || `<p style="color:${C.mu}">Desafios não cadastrados.</p>`;
...
const id  = G.id;
const full = `<div style="position:absolute;inset:0;background:linear-gradient(135deg,${C.a1},${C.a2})"></div> <div style="position:absolute;inset:0;background:${C.bg}cc"></div>`;
const body = ` <div style="margin:auto;text-align:center"> <div style="width:64px;height:64px;border-radius:14px;background:${C.a1};margin:0 auto 28px;display:flex;align-items:center;justify-content:center;font-size:30px;font-weight:900;color:${C.bg}">W</div> <div style="font-size:72px;font-weight:800;color:${C.txt};letter-spacing:-2px;margin-bottom:20px">Obrigado</div> <div style="width:56px;height:3px;background:linear-gradient(90deg,${C.a1},${C.a2});border-radius:2px;margin:0 auto 22px"></div> <div style="font-size:20px;color:${C.mu}">${safe(id.instName,'')}</div> <div style="font-size:15px;color:${C.mu};margin-top:8px">${safe(id.presDate,'')}</div> </div>`;
return `<div style="width:${PDF_W}px;height:${PDF_H}px;position:relative;overflow:hidden;font-family:${PDF_FONT}">
${full}
