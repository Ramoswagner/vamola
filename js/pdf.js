...
// Família visual: como o HTML vai se parecer (5 grupos, não 11)
function _family(m) {
...
function _sCapa(C, fam) {
  const id = G.id;
  // capa ignora a divisão header/body — usa layout especial
  let decor;
  if (fam === 'classic') {
  ...
  const tc = fam === 'header' ? 'rgba(0,0,0,.9)' : C.txt;
  const mc = fam === 'header' ? 'rgba(0,0,0,.5)' : C.mu;
  ...
  const body = `
    <div style="margin-top:auto;margin-bottom:auto">
      ...
      <div style="font-size:${titlePx}px;font-weight:800;line-height:1.05;letter-spacing:-2px;color:${tc};margin-bottom:22px;max-width:780px">${safe(id.presTitle,'Título da Apresentação')}</div>
      <div style="width:64px;height:4px;background:linear-gradient(90deg,${C.a1},${C.a2});border-radius:2px;margin-bottom:22px"></div>
